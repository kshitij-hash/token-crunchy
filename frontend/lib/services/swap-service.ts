...
import { publicClient, treasuryWalletClient, monadTestnet } from '../web3/monad-provider'
...
export class SwapService {
  ...
  private static validateApiKey(): void {
    if (!process.env.NEXT_PUBLIC_ZEROX_API_KEY) {
      throw new Error('0x API key not configured. Please set NEXT_PUBLIC_ZEROX_API_KEY environment variable.')
    }
  }
  ...
  static async getQuote(params: {
    ...
    console.log('Requesting 0x quote with params:', {
    ...
    console.log('0x quote response:', quote)

    // Check if there's actually liquidity available
    if (!quote.liquidityAvailable) {
      throw new Error('INSUFFICIENT_ASSET_LIQUIDITY: No liquidity available for this pair')
    }
    ...
    if (!quote.to && !quote.data) {
      console.warn('Quote missing transaction data, this may indicate limited testnet support')
      // Add minimal transaction data for allowance-holder pattern
      quote.to = quote.allowanceTarget || '0x0000000000001ff3684f28c67538d4d072c22734'
  ...
  static async checkAndApproveAllowance(
    ...
    try {
      ...
      if (currentAllowance === BigInt(0)) {
        console.log(`Approving AllowanceHolder to spend token ${tokenAddress}...`)
        ...
        console.log('Token approved successfully:', receipt.transactionHash)
      } else {
        console.log('Token already approved for AllowanceHolder')
      }
    } catch (error) {
      console.error('Error checking/approving allowance:', error)
      throw new Error(`Failed to approve token allowance: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
  ...
  static async executeSwap(quote: SwapQuote, _userAddress: Address) {
    ...
    if (quote.issues?.allowance !== null && quote.issues?.allowance) {
      console.log('Token approval needed for AllowanceHolder')
      ...
      console.log('Token already approved or no approval needed')
    }
    ...
    try {
      ...
      if (quote.transaction) {
        console.log('Using transaction data from quote response')
        ...
        try {
          ...
          console.log('Transaction sent successfully via sendTransaction:', txHash)
          ...
          const errorMessage = sendTxError instanceof Error ? sendTxError.message : String(sendTxError)
          const errorCode = (sendTxError as { code?: number }).code
          if (errorMessage.includes('Unsupported method: eth_sendTransaction') || 
              errorMessage.includes('method is not supported') || 
              errorCode === -32604 || 
              errorCode === -32600) {
            
            console.log('sendTransaction not supported, trying sendRawTransaction approach...')
            ...
            try {
              ...
              console.log('Preparing transaction for signing:', txRequest)
              
              // Sign the transaction
              if (!treasuryWalletClient.account) {
                throw new Error('Treasury wallet account not available for signing')
              }
              ...
              console.log('Transaction sent successfully via sendRawTransaction:', txHash)
            } catch (rawTxError: unknown) {
              console.error('sendRawTransaction also failed:', rawTxError)
              const rawErrorMessage = rawTxError instanceof Error ? rawTxError.message : String(rawTxError)
              throw new Error(`Both sendTransaction and sendRawTransaction failed. Monad testnet RPC has very limited method support. Error: ${rawErrorMessage}`)
            }
          } else {
      ...
      if (errorMessage.includes('method is not supported') || errorCode === -32604) {
        throw new Error('Monad testnet RPC has limited method support. The swap cannot be executed directly through this RPC endpoint. Please use a different RPC or signing method.')
      }
      
      // If the transaction data is empty (0x), this might be a quote-only response
      if ((quote.transaction?.data || quote.data) === '0x') {
        throw new Error('Cannot execute swap: Quote does not contain valid transaction data. This may indicate limited testnet support for the 0x allowance-holder API.')
      }
   ...
   */
  static async getAvailablePairs(): Promise<Array<{
    from: TokenSymbol
    to: TokenSymbol
    sellToken: string
    buyToken: string
    ...
    try {
      ...
      console.log('WMON->USDT pair not available:', error instanceof Error ? error.message : 'Unknown error')
    }
    ...
    try {
      ...
      console.log('USDT->WMON pair not available:', error instanceof Error ? error.message : 'Unknown error')
    }
